      ...
      <div className="pt-24 px-6 max-w-4xl mx-auto pb-8">
        ...
        <FadeIn>
          <section className="mb-10 text-center">
            ...
            <h1 className="text-4xl font-extrabold font-headline text-on-surface mb-4 tracking-tight leading-tight">
              Psikolojik Testler
            </h1>
            <p className="text-on-surface-variant text-lg leading-relaxed max-w-xl mx-auto">
              Testler, görüşmelerimizin size özel olmasını sağlar. Sonuçlarınız psikologunuz tarafından titizlikle incelenir ve seanslarınızda birlikte değerlendirilir.
            </p>
          </section>
        </FadeIn>
        ...
        <StaggerInView className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-10" stagger={0.1}>
          <StaggerItem>
            <motion.div
              whileHover={{ y: -3, boxShadow: "0 12px 40px rgba(41,104,104,0.06)" }}
          ...
          <StaggerItem>
            <motion.div
              ...
              whileHover={{ y: -3, boxShadow: "0 12px 40px rgba(78,98,108,0.06)" }}
              whileTap={{ scale: 0.97 }}
              transition={{ type: "spring", stiffness: 400, damping: 25 }}
          ...
          <StaggerItem>
            <motion.div
              whileHover={{ y: -3, boxShadow: "0 12px 40px rgba(79,99,97,0.06)" }}
              transition={{ type: "spring", stiffness: 400, damping: 25 }}
            ...
            <div className="relative z-10">
              ...
              <div className="space-y-4 text-sm text-on-surface-variant leading-relaxed">
                <div className="flex gap-3">
                  ...
                  <p><strong className="text-on-surface">Görüşmeler size özel olur.</strong> Psikologunuz test sonuçlarınızı titizlikle inceler ve seansınızı sizin ihtiyaçlarınıza göre planlar. Genel bir sohbet yerine, doğrudan sizi ilgilendiren konulara odaklanırız.</p>
                </div>
                <div className="flex gap-3">
                  <span className="material-symbols-outlined text-primary text-lg shrink-0 mt-0.5">speed</span>
                  <p><strong className="text-on-surface">Süreciniz hızlanır.</strong> Testler, görüşmelerde "nereden başlayalım?" sorusunu ortadan kaldırır. Psikologunuz durumunuzu önceden anlayarak ilk seanstan itibaren verimli bir şekilde çalışmaya başlar.</p>
                </div>
                <div className="flex gap-3">
        ...
        <FadeInView>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 text-center mb-8">
            ...
            <span className="material-symbols-outlined text-outline-variant/30 text-sm hidden sm:block">arrow_forward</span>
            <Step n="3" text="Seansınızda birlikte konuşulur" icon="handshake" />
          </div>
        </FadeInView>
...
function TestCard({ icon, badge, title, subtitle, desc, href, pdfUrl, duration }: {
  icon: string; badge: string; title: string; subtitle: string; desc: string; href: string; pdfUrl: string; duration: string
}) {
  return (
    <motion.div
      whileHover={{ y: -6 }}
      transition={{ type: "spring", stiffness: 300, damping: 25 }}
      className="group relative flex flex-col bg-surface-lowest p-8 rounded-[2.5rem] transition-shadow duration-500 hover:shadow-[0_40px_80px_rgba(41,104,104,0.08)] overflow-hidden border border-white/40 h-full"
    >
      ...
      <div className="space-y-3">
        <Link href={href}
          className="w-full h-14 bg-primary text-on-primary rounded-2xl font-bold flex items-center justify-center gap-2 transition-all duration-300 hover:shadow-lg hover:shadow-primary/20 active:scale-95">
